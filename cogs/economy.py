...
class Economy(commands.Cog):
    ...
    async def ranking_graczy(self, interaction: discord.Interaction):
        ...
        for i, row in enumerate(rows, start=1):
            lines.append(f"{i}. {row['username']} — {row['balance']} pkt")
    ...
    async def moje_zaklady(self, interaction: discord.Interaction):
        ...
        for b in bets:
            if b["status"] != "settled":
                ...
                status = f"✅ wygrana ({int(round(b['amount'] * b['odds']))} pkt)"
            else:
                status = "❌ przegrana"
            lines.append(
                f"#{b['match_id']} {b['player_a']} vs {b['player_b']} — "
                f"typ: {b['player_choice']} @ {b['odds']} — {b['amount']} pkt — {status}"
            )
